  ...
  <div class="dashboard-bg">
      ...
      <div class="container mt-0 pt-6 px-4 pb-6">
        ...
        <div class="level mb-6 glass-panel p-5">
          <div class="level-left">
            <button class="button is-ghost has-text-white p-0 mr-5" @click="volver"><span class="icon is-large"><i class="fas fa-arrow-left fa-lg"></i></span></button>
            <div>
              ...
              <div class="field mb-5">
                ...
                <div class="control has-icons-left">
                  <div class="select is-fullwidth is-dark" :class="{'is-disabled': !esDocente}">
                    <select v-model.number="form.estado_id" :disabled="!esDocente"><option :value="null" disabled>Seleccione un estado</option><option v-for="est in estadosProyecto" :key="est.id" :value="est.id">{{ est.nombre }}</option></select>
                  </div>
                  <span class="icon is-left has-text-info"><i class="fas fa-signal"></i></span>
                </div>
              </div>
              <div class="field mb-5"><label class="label has-text-grey-lighter is-small uppercase-label">Fecha 1er Cierre</label><div class="control"><input class="input is-dark" type="date" v-model="form.fecha_cierre_1" :disabled="!esDocente"></div></div>
              <div class="field mb-5"><label class="label has-text-grey-lighter is-small uppercase-label">Fecha 2do Cierre</label><div class="control"><input class="input is-dark" type="date" v-model="form.fecha_cierre_2" :disabled="!esDocente"></div></div>
              <div class="field mt-6"><label class="label compromise-label has-text-grey-lighter is-small uppercase-label">Notas del Proyecto</label><textarea class="textarea is-dark" rows="10" v-model="form.descripcion" :disabled="!esDocente" placeholder="Notas adicionales..."></textarea></div>
            </div>
          </div>
            ...
            <div class="box glass-panel p-0 is-flex is-flex-direction-column" style="min-height: 600px;">
              ...
              <div class="p-5 flex-grow-1">
                <div v-if="tabActiva === 'alcance'" class="animate__animated animate__fadeIn">
                  <div class="field mb-6"><label class="label has-text-white is-size-6 mb-3 uppercase-label">TÍTULO DEL PROYECTO</label><div class="control"><input class="input is-dark is-medium custom-input-title" type="text" v-model="form.nombre" :disabled="!esDocente"></div></div>
                  <div v-for="campo in camposAlcance" :key="campo.key" class="field mb-6">
                    <div class="is-flex is-justify-content-between is-align-items-center mb-3">
                      ...
                      <button v-if="esDocente" class="button is-small" :class="form[campo.key + 'Bloqueado'] ? 'is-danger' : 'is-success is-outlined'" @click="form[campo.key + 'Bloqueado'] = !form[campo.key + 'Bloqueado']" type="button"><span class="icon is-small"><i :class="form[campo.key + 'Bloqueado'] ? 'fas fa-lock' : 'fas fa-lock-open'"></i></span><span>{{ form[campo.key + 'Bloqueado'] ? 'Bloqueado' : 'Abierto' }}</span></button>
                    </div>
                    <div class="control"><textarea class="textarea is-dark custom-textarea" rows="6" v-model="form[campo.key]" :disabled="!esDocente && form[campo.key + 'Bloqueado']" :placeholder="(!esDocente && form[campo.key + 'Bloqueado']) ? 'Contenido bloqueado por el docente' : 'Escriba aquí...'"></textarea></div>
                  </div>
                </div>
                ...
                <div v-if="tabActiva === 'equipo'" class="animate__animated animate__fadeIn">
                  <div class="field mb-5 buscador-relativo" v-if="esDocente">
                    <div class="control has-icons-left">
                      ...
                      <div v-if="resultadosBusqueda.length > 0" class="search-results-floating box p-0"><a v-for="u in resultadosBusqueda" :key="u.id" @click="seleccionarUsuario(u)" class="dropdown-item-custom"><div class="is-flex is-justify-content-between is-align-items-center"><span><strong>{{ u.apellido?.toUpperCase() }}</strong>, {{ u.nombre }}</span><span class="tag is-small is-dark">{{ Number(u.rol_id) === 3 ? 'Alumno' : 'Docente' }}</span></div></a></div>
                    </div>
                  </div>
                      ...
                      <div class="box p-3 is-dark-box">
                        <article class="media is-align-items-center">
                          ...
                          <div class="media-right">
                              <div class="buttons">
                                  <button v-if="Number(miembro.rol_id) === 3" class="button is-ghost has-text-info p-0 mr-3" @click="intentarVerDetalle(miembro)" title="Ver detalle de notas"><i class="fas fa-eye"></i></button>
                                  <button v-if="esDocente && Number(miembro.rol_id) === 3" class="button is-ghost has-text-info p-0 mr-3" @click="abrirModalSeguimiento(miembro)" title="Agregar Seguimiento"><i class="fas fa-chart-line"></i></button>
                                  <button v-if="esDocente" class="button is-ghost has-text-danger p-0" @click="confirmarEliminacionUsuario(miembro)" title="Eliminar usuario"><i class="fas fa-user-minus"></i></button>
                              </div>
                          </div>
                        </article>
                      </div>
                ...
                <div v-if="tabActiva === 'viabilidad'" class="animate__animated animate__fadeIn">
                  <div class="box is-dark-box p-5 mb-5"><div class="field"><label class="label has-text-white is-size-5 uppercase-label">Aprobación de Anteproyecto</label><div class="control mt-4"><label class="checkbox has-text-white is-size-4" :class="{'is-disabled': !esDocente}"><input type="checkbox" v-model="form.viable" class="mr-3" :disabled="!esDocente"> PROYECTO VALIDADO / VIABLE</label></div></div></div>
                  <div class="field mb-5">
                    <label class="label has-text-white uppercase-label">Documentación de Respaldo</label>
                    <div class="field has-addons mt-3">
                      <div class="control is-expanded has-icons-left"><input class="input is-dark is-medium" type="text" v-model="form.documentoViabilidadLink" placeholder="Link del documento..." :disabled="!esDocente"><span class="icon is-left has-text-info"><i class="fas fa-link"></i></span></div>
                      <div class="control"><button class="button is-info is-medium" :disabled="!form.documentoViabilidadLink" @click="abrirEnlace(form.documentoViabilidadLink)"><i class="fas fa-external-link-alt"></i></button></div>
                    </div>
                    <p class="help class has-text-grey-lighter is-size-6 mt-2">Link al archivo digitalizado que acredita la aprobación.</p>
                  </div>
                  <article v-if="form.viable && !form.documentoViabilidadLink" class="message is-warning is-small mb-6"><div class="message-body"><strong>Falta Respaldo:</strong> No se puede considerar viable sin el link al documento digitalizado.</div></article>
                  ...
                  <div class="box is-dark-box p-5 mb-5" v-if="esDocente">
                    ...
                    <form @submit.prevent="crearCalificacionDiaria">
                      <div class="columns">
                        <div class="column is-3"><div class="field"><label class="label has-text-grey-light is-small uppercase-label">Fecha Evaluada</label><div class="control"><input class="input is-dark" type="date" v-model="notaForm.fecha_evaluacion" required></div></div></div>
                  ...
                  <div class="box is-dark-box p-5 mb-4">
                    <div class="is-flex is-justify-content-between is-align-items-center mb-2"><h4 class="title is-6 has-text-white uppercase-label mb-0"><i class="fas fa-chart-bar mr-2"></i> Estado de Rendimiento Grupal</h4><span class="tag is-medium has-text-weight-bold" :class="obtenerColorNota(promedioProyecto)">PROMEDIO: {{ promedioProyecto }} / 10</span></div>
                    <progress class="progress is-medium mt-2" :class="obtenerColorNota(promedioProyecto)" :value="promedioProyecto" max="10"></progress>
                  </div>
                  <div class="box is-dark-box p-0 mb-5 field-accordion">
                    <button class="button is-dark is-fullwidth is-flex is-justify-content-between is-align-items-center p-5 custom-accordion-btn" type="button" @click="accordionAbierto = !accordionAbierto"><span class="has-text-weight-bold uppercase-label has-text-info"><i class="fas fa-history mr-2"></i> Ver Desglose de Evaluaciones Diarias ({{ notasHistorial.length }})</span><span class="icon"><i :class="accordionAbierto ? 'fas fa-chevron-up' : 'fas fa-chevron-down'"></i></span></button>
                    <div v-if="accordionAbierto" class="p-4 animate__animated animate__slideInDown animate__faster container-table-accordion">
                      ...
                      <div class="table-container" v-else>
                        <table class="table is-fullwidth glass-table delivery-table-v2 mb-0">
                          <thead><tr><th class="has-text-info is-size-7 uppercase-label th-compacta-fecha">FECHA</th><th class="has-text-info is-size-7 uppercase-label th-compacta-concepto">CONCEPTO</th><th class="has-text-info is-size-7 uppercase-label has-text-centered">NOTA</th><th class="has-text-info is-size-7 uppercase-label">EVALUADOR</th><th class="has-text-info is-size-7 uppercase-label">MOTIVO / DETALLE</th><th class="has-text-info is-size-7 uppercase-label has-text-centered">ACCIONES</th></tr></thead>
                          <tbody>
                            <tr v-for="nota in notasHistorial" :key="nota.id">
                              ...
                              <td class="has-text-centered"><div class="buttons is-centered mb-0" v-if="esDocente"><button class="button is-small is-ghost has-text-info p-1" @click="prepararEdicionNotaGrupal(nota)" title="Editar nota grupal"><i class="fas fa-edit"></i></button><button class="button is-small is-ghost has-text-danger p-1" @click="prepararEliminacionNotaGrupal(nota)" title="Eliminar nota grupal"><i class="fas fa-trash-alt"></i></button></div></td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                ...
                <div v-if="tabActiva === 'entregables'" class="animate__animated animate__fadeIn">
                  <div class="box is-dark-box p-4 mb-6">
                    ...
                    <div class="field has-addons mt-3">
                      ...
                      <div class="control"><button class="button is-info" :disabled="!form.linkDrive" @click="abrirEnlace(form.linkDrive)" title="Abrir carpeta"><i class="fas fa-external-link-alt"></i></button></div>
                    </div>
    ...
    <div class="modal" :class="{'is-active': mostrarModalConfirmacion}">
      ...
      <div class="modal-card">
        ...
        <section class="modal-card-body">¿Estás seguro de que deseas eliminar? Esta acción no se puede deshacer.</section>
...
.custom-textarea { width: 100% !important; max-width: 100% !important; background: rgba(0, 0, 0, 0.4) !important; color: white !important; border: 1px solid rgba(255, 255, 255, 0.2) !important; resize: vertical; }
.custom-input-title { background: rgba(52, 152, 219, 0.15) !important; color: #3498db !important; border: 1px solid rgba(52, 152, 219, 0.4) !important; font-weight: 700 !important; text-transform: uppercase; }
...
.delivery-table-v2 td, .delivery-table-v2 th { background-color: transparent !important; border-bottom: 1px solid rgba(255,255,255,0.05) !important; color: white; vertical-align: middle; }
